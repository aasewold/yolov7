...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader = create_dataloader(test_path, imgsz_test, opt.val_batch_size, gs, opt,  # testloader
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '), max_negative=1.0)[0]
    ...
    try:
        for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                plot_epoch = (epoch % opt.plot_interval) == 0
    ...
    if rank in [-1, 0]:
        ...
        for f in last, best:
            if f.exists():
                strip_optimizer(str(f))  # strip optimizers
        if opt.bucket:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-negative', type=float, default=1.0, help='Maximum fraction of negative samples')
    parser.add_argument('--plot-interval', type=int, default=10, help='Make plots after every "plot_interval" epoch')
    parser.add_argument('--val-batch-size', type=int, default=None, help='Batch size for validation')
    opt = parser.parse_args()

    opt.val_batch_size = opt.val_batch_size or opt.batch_size
